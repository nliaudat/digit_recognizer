...
"""
esp_quantization_ready_v2_aggressive – Maximum-Accuracy ESP-DL CNN (>97% target)
==================================================================================
Design goal: Push accuracy as high as possible within the ESP-DL operator
whitelist. Uses high-capacity conv blocks (64 → 128 filters) and a large
Dense head (256 units), accepting a larger model footprint for extra accuracy.

Architecture:
  - Conv2D(64) × 2 + ReLU + MaxPool              → Block 1 (high capacity)
  - Conv2D(128) × 2 + ReLU + MaxPool             → Block 2 (very wide)
  - DepthwiseConv2D(3×3) + Conv2D(256, 1×1) + ReLU → DW-Sep block (final)
  - GlobalAveragePooling2D
  - Dense(256) + ReLU + Dropout(0.3) → Dense(NB_CLASSES) Softmax
