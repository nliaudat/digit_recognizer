...
def create_digit_recognizer_v30_teacher(
    ...
    try:
        ...
        # If it fails, maybe it's a version mismatch or weights loading issue
        # Try without weights first
        print(f"⚠️  EfficientNetB0 init error: {e}. Trying fallback...")
        backbone = keras.applications.EfficientNetB0(
            include_top=False,
            weights=None,
            input_shape=effnet_shape,
        ...
        if pretrained:
            print("   (Note: Pretrained weights could not be loaded safely)")
    backbone.trainable = not freeze_backbone
    features = backbone(x)

    # ── Classification head ───────────────────────────────────────────────────
    x = keras.layers.GlobalAveragePooling2D(name="gap")(features)
    x = keras.layers.Dense(512, name="dense_1")(x)
