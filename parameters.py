...
def update_derived_parameters():
    ...
    DATA_SOURCES = [
        ...
        },
        {
            ...
            'weight': 5.0,  # ~1.8k images (Corrected bad predictions, highly weighted)
        },
        {
            'name': 'real_integra',
            'type': 'label_file', 
            ...
            'weight': 3.0,  # ~1.8k images (Small dataset, weighted for balance)
        },
        {
            ...
            'weight': 5.0,  # ~1k to 4.3k images (Corrected bad predictions, highly weighted)
        },
        {
            ...
            'weight': 0.5,  # ~70k images generated by static augmentation (Large dataset, slightly down-weighted)
            'is_synthetic': True,
        ...
        },
        {
            ...
            'weight': 0.5,  # ~3.3k images (Small dataset, weighted for for mixup bad classes)
            'is_synthetic': True,
            # 'not_in_bench': True, # Not in benchmark because it is generated data
        },
        {
            ...
            'weight': 3.0,  # ~0.8k images (Very small dataset, up-weighted)
        },
...
# XNNPACK delegates are NOT supported in TFLite Micro (used on ESP32, microcontrollers, etc.).
# When True, converter.target_spec.supported_ops is restricted to TFLITE_BUILTINS/TFLITE_BUILTINS_INT8
# which prevents the DELEGATE node from being baked into the exported .tflite model.
# Also sets TF_ENABLE_XNNPACK=0 env var for tools like onnx2tf that bypass the converter API.
DISABLE_XNNPACK = True

# Force TFLITE_BUILTINS_INT8 as the only supported ops for all TFLite conversions.
# This is the most reliable way to prevent XNNPACK delegates from being baked into
# the exported .tflite model, as it forces full integer quantization which implicitly
# overrides any XNNPACK delegation. Overrides DISABLE_XNNPACK when True.
# Reference: converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS_INT8]
...
def validate_quantization_parameters():
    ...
    # (QUANTIZATION_MODE="tqt" sets ESP_DL_QUANTIZE=True at import, but CLI --qat --no-tqt
    #  overrides USE_QAT=True without resetting ESP_DL_QUANTIZE)
